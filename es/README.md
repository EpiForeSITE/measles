...
2. **Simulador de brote en una sola escuela en R**: La aplicación Shiny es un contenedor del modelo `ModelMeaslesSchool` en el paquete [`epiworldR`](https://cran.r-project.org/package=epiworldR). El paquete `epiworldR` a su vez es un contenedor de la biblioteca en C++ `epiworld` ([enlace](https://github.com/UofUEpiBio/epiworld)), que provee el motor central de simulación para todos sus modelos.
